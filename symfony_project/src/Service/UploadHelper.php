...
class UploadHelper
{
    ...
    const MASTERCLASS_VIDEO_PATH = 'uploads/masterclass_video';
    ...
    public function uploadMasterclassVideo(UploadedFile $file): string
    {
        $originalfileName = $file->getFilename();
        $basefileName = pathinfo($originalfileName, PATHINFO_FILENAME);
        $fileName = Urlizer::urlize($basefileName . '-' . uniqid() . '.' . $file->guessExtension());
        ...
        $stream = fopen($file->getPathName(), 'r');
        $this->defaultStorage->writeStream(
            self::MASTERCLASS_VIDEO_PATH . '/' . $fileName,
            $stream
        );
    ...
    public function fixtureMusicSheetUpload(File $file): string
    {
        $originalfileName = $file->getFilename();
        $basefileName = pathinfo($originalfileName, PATHINFO_FILENAME);
        $fileName = Urlizer::urlize($basefileName . '-' . uniqid() . '.' . $file->guessExtension());
    ...
    public function fixtureMasterclassVideoUpload(File $file): string
    {
        $originalfileName = $file->getFilename();
        $basefileName = pathinfo($originalfileName, PATHINFO_FILENAME);
        $fileName = Urlizer::urlize($basefileName . '-' . uniqid() . '.' . $file->guessExtension());
